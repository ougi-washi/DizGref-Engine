...
dg_vec3 vec3_sub(dg_vec3 a, dg_vec3 b){
    return (dg_vec3){ a.x-b.x, a.y-b.y, a.z-b.z };
}

dg_vec3 vec3_norm(dg_vec3 v){
    const f32 len = vec3_length(v);
    return (dg_vec3){ v.x/len, v.y/len, v.z/len };
}
...
dg_mat4 mat4_look_at(dg_vec3 eye, dg_vec3 center, dg_vec3 up) {
    dg_vec3 f = vec3_norm(vec3_sub(center, eye));
    dg_vec3 s = vec3_norm(vec3_cross(f, up));
    dg_vec3 u = vec3_cross(s, f);

    dg_mat4 M = mat4_identity();
    // rotation
