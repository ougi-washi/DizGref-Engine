...
static dg_audio_data_t dg_audio_data = {0};
...
static int dg_audio_callback(const void* input_buffer, void* output_buffer,
                         unsigned long frames_per_buffer,
                         const PaStreamCallbackTimeInfo* time_info,
                         PaStreamCallbackFlags status_flags,
                         void* user_data) {
    
    dg_audio_data_t* data = (dg_audio_data_t*)user_data;
    ...
    data->amps = (dg_vec3){low_vol, mid_vol, high_vol};
    return paContinue;
}
...
void dg_audio_init() {
    ...
    dg_audio_data.buffer = (float*)malloc(FRAMES_PER_BUFFER * sizeof(float));
    dg_audio_data.buffer_size = FRAMES_PER_BUFFER;
    
    if (!dg_audio_data.buffer) {
    ...
    err = Pa_OpenStream(&dg_audio_data.stream,
                        &input_parameters,
                        NULL,
                        SAMPLE_RATE,
                        FRAMES_PER_BUFFER,
    ...
    err = Pa_StartStream(dg_audio_data.stream);
    if (err != paNoError) {
        printf("Cannot start stream: %s\n", Pa_GetErrorText(err));
        Pa_CloseStream(dg_audio_data.stream);
        dg_audio_cleanup();
        return;
    }
    
    printf("Volume and frequency detection active...\n\n");
    dg_audio_data.running = true;
}

void dg_audio_cleanup(){
    dg_audio_data.running = false;
    PaError err;
    err = Pa_StopStream(dg_audio_data.stream);
    if (err != paNoError) {
        printf("PortAudio error: %s\n", Pa_GetErrorText(err));
        goto cleanup;
    }

    err = Pa_CloseStream(dg_audio_data.stream);
    if (err != paNoError) {
        printf("PortAudio error: %s\n", Pa_GetErrorText(err));
    }

cleanup:
    free(dg_audio_data.buffer);
    Pa_Terminate();
}


dg_vec3 dg_audio_get_amplitudes(){
    return dg_audio_data.amps;
}
